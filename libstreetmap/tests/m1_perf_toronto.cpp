...
SUITE(street_queries_perf) {
    ...
    //Add a unit test here to test the performance of "find_street_ids_from_name"
        ...
        expected =  {732, 738, 2842};
        actual = find_street_ids_from_name("McCowan Road");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {2627};
        actual = find_street_ids_from_name("Yukon Place");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {1193};
        actual = find_street_ids_from_name("Cherryhill Avenue");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {19, 6724};
        actual = find_street_ids_from_name("Don Valley Parkway Northbound");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {679};
        actual = find_street_ids_from_name("Leroy Avenue");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {9277};
        actual = find_street_ids_from_name("Janes Lane");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {12739};
        actual = find_street_ids_from_name("Halesia Drive");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {12777};
        actual = find_street_ids_from_name("Dashwood Crescent");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {2356};
        actual = find_street_ids_from_name("Whytock Avenue");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {865, 19047};
        actual = find_street_ids_from_name("Kingston Road East");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        ...
        expected = { 473, 3116, 3119, 3120, 3122, 3438, 4587, 4588, 4824};
        actual = find_street_ids_from_name("To Highway 401 Collectors");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {164, 179, 180};
        actual = find_street_ids_from_name("Highway 427 Express");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        ...
        for (size_t i = 0; i < 8000000; i++) {
            street_names.push_back(getStreetName(i));
        }
        {
            //Timed Test
            ECE297_TIME_CONSTRAINT(70);
