...
std::vector<unsigned> find_street_ids_from_name(std::string street_name);//completed .. not unit tested

//function to return the street segments for a given intersection 
std::vector<unsigned> find_intersection_street_segments(unsigned intersection_id); //performance test failed

//function to return street names at an intersection (include duplicate street names in returned vector)
std::vector<std::string> find_intersection_street_names(unsigned intersection_id); // ..working
...
bool are_directly_connected(unsigned intersection_id1, unsigned intersection_id2); //...  PAss

//---------------------Thursday, 21st Jan ---------------------
//find all intersections reachable by traveling down one street segment 
//from given intersection (hint: you can't travel the wrong way on a 1-way street)
//the returned vector should NOT contain duplicate intersections
std::vector<unsigned> find_adjacent_intersections(unsigned intersection_id); // passed

//for a given street, return all the street segments
std::vector<unsigned> find_street_street_segments(unsigned street_id); //working

//for a given street, find all the intersections
std::vector<unsigned> find_all_street_intersections(unsigned street_id); //working
...
std::vector<unsigned> find_intersection_ids_from_street_names(std::string street_name1, std::string street_name2); //working
...
//-------------------Sunday, 24th Jan ---------------------------------------
//Pankaj is working on it
//find the length of a whole street
double find_street_length(unsigned street_id); // pass

//find the travel time to drive a street segment (time(minutes) = distance(km)/speed_limit(km/hr) * 60
double find_street_segment_travel_time(unsigned street_segment_id); //Panjak SIngh .. working 

//find the nearest point of interest to a given position
unsigned find_closest_point_of_interest(LatLon my_position); // working

//find the nearest intersection (by ID) to a given position
unsigned find_closest_intersection(LatLon my_position); //performance failed
